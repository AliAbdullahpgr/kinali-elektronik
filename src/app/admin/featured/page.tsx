...
export default function FeaturedProductsPage() {
  ...
  const filteredNonFeatured = nonFeaturedProducts.filter(
    (p) =>
      turkishIncludes(p.title, searchQuery) ||
      turkishIncludes(p.productCode, searchQuery)
  );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Öne Çıkan Ürünler</h1>
        <p className="text-sm text-gray-500 mt-1">
          Ana sayfada görünecek ürünleri seçin. Öne çıkan ürünler ana sayfada
          vurgulanır.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">
            Şu Anda Öne Çıkan ({featuredProducts.length})
          </h2>
          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
            Maksimum 8 ürün önerilir
          </span>
        </div>
        ...
        {featuredProducts.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">Henüz öne çıkan ürün yok.</p>
            <p className="text-xs mt-1">Aşağıdan ürün seçerek öne çıkarabilirsiniz.</p>
          </div>
        ) : (
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">
            Tüm Ürünler ({nonFeaturedProducts.length})
          </h2>
        ...
        {filteredNonFeatured.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            <p className="text-sm">
              {searchQuery ? "Arama sonucu bulunamadı." : "Tüm ürünler zaten öne çıkarılmış."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
